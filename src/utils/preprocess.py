...
class WikiPreprocessor:
    ...
    def _read_directory(self, directory, source=False):
        # below is to ensure previous files are removed/overwriten since we are appending
        if os.path.exists(self.output_directory+'source_packed' + str(self.rank) + '.txt'):
            os.remove(self.output_directory+'source_packed' + str(self.rank) + '.txt') 
