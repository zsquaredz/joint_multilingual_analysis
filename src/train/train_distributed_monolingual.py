import torch, os
from transformers import RobertaConfig, RobertaTokenizer, RobertaForMaskedLM, XLMRobertaConfig, XLMRobertaTokenizer, XLMRobertaForMaskedLM
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tokenizer_path', type=str, help='path to tokenizer', default='roberta-base')
    parser.add_argument('--use_multi_tokenizer', action='store_const', const=True, default=False)
    ...
    parser.add_argument('--langs_to_use', nargs='+', type=str, default=["en"])
    ...
    if not args.use_multi_tokenizer:
        logging.info('Using a monolingual tokenizer')
        tokenizer = RobertaTokenizer.from_pretrained(args.tokenizer_path, max_len=args.max_seq_len, padding='max_length')
    else:
        logging.info('Using a multiligual tokenizer')
    ...
    if args.load_from_checkpoint:
        ...
        model = RobertaForMaskedLM.from_pretrained(args.load_from_checkpoint)
    else:
        logging.info('Initializing new random Roberta model')
        config = RobertaConfig.from_pretrained('roberta-base')
       
        config.vocab_size = tokenizer.vocab_size # make sure that vocab_size is the same as the tokenizer
